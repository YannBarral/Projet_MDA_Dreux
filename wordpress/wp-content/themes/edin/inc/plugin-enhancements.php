...
class Theme_Plugin_Enhancements {
	...
	function __construct() {
		...
		if ( empty( $this->dependencies ) ) :
			...
			$this->plugins = array(
	    		...
	    		'message' => sprintf(
					__( 'The %1$s is required to use some of this theme’s features, including: ', 'edin' ),
				'<strong>' . __( 'Jetpack plugin', 'edin' ) . '</strong>' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			   'none'                 => __( 'no specific module needed', 'edin' ),
			   'custom-content-types' => __( 'Custom Content Types module', 'edin' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please activate %1$s. %2$s', 'edin' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . __( 'Activate', 'edin' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please install %1$s. %2$s', 'edin' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . __( 'Install', 'edin' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				__( 'To use %1$s, please activate the Jetpack plugin’s %2$s.', 'edin' ),
